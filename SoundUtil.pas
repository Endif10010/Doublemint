...
begin
  ...
  //DebugOutStr('g_sMapMusic1: ' + g_sMapMusic);
  if boFlag then begin
    //DebugOutStr('g_sMapMusic2: ' + g_sMapMusic);
    if g_sMapMusic <> '' then begin
      if CompareLStr(g_sMapMusic, 'HTTP://', Length('HTTP://')) then begin
        nIndex := 1;
      end else begin
    ...
    if Assigned(g_PlugInfo.MediaPlayer.StopPlay) then begin
      g_PlugInfo.MediaPlayer.StopPlay(PAnsiChar(sFileName));
    end;
  end;
  ...
  PlayMp3('', False);
  //DebugOutStr('g_sMapMusic3: ' + IntToStr(nIndex));
  case nIndex of
    1: begin
        if boFlag then begin
          //DebugOutStr('g_sMapMusic4: ' + g_sMapMusic);
          frmMain.PlayMedia(sFileName);
          //SoundEngine.MediaPlayer(sFileName);
         { if Assigned(g_PlugInfo.MediaPlayer.Player) then begin
            g_PlugInfo.MediaPlayer.Player(PAnsiChar(sFileName), False, True);
          end; }
        end;
      end;
    2: begin
