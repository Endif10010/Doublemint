...
uses
  ...
  DWinCtl, StdCtrls, Textures, Grids, Grobal2, ClFunc, HUtil32, Actor, GameImages,
...
//    m_Dicea:array[0..35] of Integer;
    ...
    procedure DrawBodyItemEffect(dsurface: TTexture; X, Y: Integer; Item: pTStdItem; Effect: Pointer); overload;
    procedure DrawBodyItemEffect(dsurface: TTexture; ARect: TRect; Item: pTStdItem; Effect: Pointer); overload; //画出装备特效
    procedure DrawBodyItem(dsurface: TTexture; ARect: TRect; Item: pTStdItem; Effect: Pointer);
    procedure DrawGridStdItem(dsurface: TTexture; ARect: TRect; Item: pTStdItem; Effect: Pointer);
    procedure DrawGridItem(dsurface: TTexture; ARect: TRect; Item: PTClientItem; Effect: Pointer);
  end;
...
begin
  ...
  try
    ...
    if CompareLStr(sClickString, '@HTTP://', Length('@HTTP://')) then begin
      ...
      if CompareLStr(sClickString, '@RMST://', Length('@RMST://')) then begin
      ...
      if (sMsg <> '') and (Str <> '') then begin
        ...
        if Assigned(g_PlugInfo.MediaPlayer.Player) then begin
          g_PlugInfo.MediaPlayer.Player(PAnsiChar(g_sDownSongAddr), False, True);
        end;

      end;

    end else
      if CompareLStr(sClickString, '@MEDIA://', Length('@MEDIA://')) then begin
      sMsg := Copy(sClickString, 10, Length(sClickString) - 9);
      if Assigned(g_PlugInfo.MediaPlayer.Player) then begin
        g_PlugInfo.MediaPlayer.Player(PAnsiChar(sMsg), True, True);
      end;
    end else begin
      nCode := 3;
...
function FastCharPos(const aSource: string; const C: AnsiChar; StartPos: Integer): Integer;
var
  L: Integer;
begin
...
begin
  ...
  if g_boStartMoveItemValue and (not DItemBag.Visible) then
    frmMain.SendClientMessage(CM_STOPMOVEITEMVALUE, 0, 0, 0, 0);
end;
    ...
    case StatePage of
      0: begin
          ...
          if g_UseItems[U_DRESS].s.Name <> '' then begin
            ...
            DrawBodyItemEffect(dsurface, SurfaceX(bbx), SurfaceY(bby), @g_UseItems[U_DRESS].s, @g_UseItemsEffect[U_DRESS]);
          end;
          if g_UseItems[U_WEAPON].s.Name <> '' then begin
            ...
            if Idx >= 0 then begin
              ...
              DrawBodyItemEffect(dsurface, SurfaceX(bbx), SurfaceY(bby), @g_UseItems[U_WEAPON].s, @g_UseItemsEffect[U_WEAPON]);
          ...
          if g_UseItems[U_HELMET].s.Name <> '' then begin
            ...
            //DrawBodyItemEffect(dsurface, SurfaceX(bbx), SurfaceY(bby), @g_UseItems[U_HELMET].s, @g_UseItemsEffect[U_HELMET]);
            DrawBodyItemEffect(dsurface, DSWHelmet.VirtualRect, @g_UseItems[U_HELMET].s, @g_UseItemsEffect[U_HELMET]);
          end;
        end;
      1: begin //瓷仿摹
          L := Left + 112; //66;
...
begin
  nWhere := -1;
  if StatePage = 0 then begin
    nWhere := TDButton(Sender).Tag;
    if nWhere >= 0 then begin
      if (nWhere in [U_DRESS, U_WEAPON]) then
        DrawGridItem(dsurface, TDButton(Sender).VirtualRect, @g_UseItems[nWhere].s, @g_UseItemsEffect[nWhere])
      else
        DrawBodyItem(dsurface, TDButton(Sender).VirtualRect, @g_UseItems[nWhere].s, @g_UseItemsEffect[nWhere]);
    end;
    {
     ...
     if nWhere >= 0 then begin
      if g_UseItems[nWhere].s.Name <> '' then begin
        ...
        if Idx >= 0 then begin
          ...
          if (g_UseItems[nWhere].s.AddValue[12] in [1, 3]) then begin
...
begin
  ...
  if g_boItemMoving then begin
    ...
    if sel in [Low(TUseItems)..High(TUseItems)] then begin
...
begin
  ...
  if MouseItem.s.Name <> '' then begin
    HintList.AddObject({'★' + } MouseItem.s.Name, {TObject(clyellow)} TObject(MakeWord(MouseItem.s.AddValue[13],MouseItem.s.Color)));
    if (g_ServerConfig.btShowClientItemStyle = 0) then begin
    ...
    if (g_ServerConfig.btShowClientItemStyle = 0) then begin
      if g_ServerConfig.boAllowItemAddPoint then begin
        ...
        if MouseItem.s.AddPoint[0] > 0 then HintList.AddObject('暴击机率增加: ' + IntToStr(MouseItem.s.AddPoint[0]) + '%', TObject(clLime));
        if MouseItem.s.AddPoint[3] > 0 then HintList.AddObject('物理防御增强: ' + IntToStr(MouseItem.s.AddPoint[3]) + '%', TObject(GetRGB(68)));
        if MouseItem.s.AddPoint[4] > 0 then HintList.AddObject('魔法防御增强: ' + IntToStr(MouseItem.s.AddPoint[4]) + '%', TObject(GetRGB(68)));
        if MouseItem.s.AddPoint[5] > 0 then HintList.AddObject('物理攻击增强: ' + IntToStr(MouseItem.s.AddPoint[5]) + '%', TObject(GetRGB(68)));
        if MouseItem.s.AddPoint[6] > 0 then HintList.AddObject('魔法攻击增强: ' + IntToStr(MouseItem.s.AddPoint[6]) + '%', TObject(GetRGB(68)));
        if MouseItem.s.AddPoint[7] > 0 then HintList.AddObject('道术攻击增强: ' + IntToStr(MouseItem.s.AddPoint[7]) + '%', TObject(GetRGB(68)));
...
begin
  ...
  if ssRight in Shift then begin
    ...
    if nIdx in [6..High(TItemArr)] then begin
      ...
      if (g_MouseItem.s.Name <> '') then begin
        if (g_ServerConfig.btShowClientItemStyle = 0) then begin
          ...
          if g_Config.boCompareItem and (nWhere in [Low(TUseItems)..High(TUseItems)]) then begin
  ...
  if nIdx in [6..High(TItemArr)] then begin
    if not g_boItemMoving then begin
      ...
      if (ssCtrl in Shift) then begin
        if g_ItemArr[nIdx].s.Name <> '' then begin
          ...
          if EdChat.SelStart = Length(EdChat.Text) then begin
            EdChat.Text := EdChat.Text + '<0|' + g_MySelf.m_sUserName + '|' + g_ItemArr[nIdx].s.Name + '|' + IntToStr(g_ItemArr[nIdx].MakeIndex) + '|' + IntToStr(g_ItemArr[nIdx].s.looks) + '>';
            ...
            sData2 := '<0|' + g_MySelf.m_sUserName + '|' + g_ItemArr[nIdx].s.Name + '|' + IntToStr(g_ItemArr[nIdx].MakeIndex) + '|' + IntToStr(g_ItemArr[nIdx].s.looks) + '>';
      ...
      if (g_MovingItem.Owner = DItemBag) or (g_MovingItem.Owner = DDealDlg) or (g_MovingItem.Owner = DUpgrade) or (g_MovingItem.Owner = DStoreDlg) then begin
        if (g_ItemArr[nIdx].s.Name <> '') then begin
          if g_boStartMoveItemValue then begin //开始转移装备属性
            frmMain.SendMoveItemValue(g_MovingItem.Item.MakeIndex, g_ItemArr[nIdx].MakeIndex);
            g_WaitingMoveItemValue := g_MovingItem;
            g_MovingItem.Item.s.Name := '';
            g_boItemMoving := False;
            g_MovingItem.Owner := nil;
          end else begin
...
begin
  ...
  if Idx in [6..High(TItemArr)] then begin
    if g_ItemArr[Idx].s.Name <> '' then begin
...
begin
  nIdx := ACol + ARow * DItemGrid.ColCount + 6;
  if nIdx in [6..High(TItemArr)] then begin
    DrawGridItem(dsurface, Bounds(DItemGrid.SurfaceX(Rect.Left), DItemGrid.SurfaceY(Rect.Top), DItemGrid.ColWidth, DItemGrid.RowHeight), @g_ItemArr[nIdx], @g_ItemArrEffect[nIdx]);
  end;
end;
{
var
...
begin
  ...
  if nIdx in [6..High(TItemArr)] then begin
    if g_ItemArr[nIdx].s.Name <> '' then begin
      ...
      with DItemGrid do begin
        if (g_ItemArr[nIdx].s.AddValue[12] in [1, 3])  then begin
...
begin
  ...
  if nIdx in [0..9] then begin
    DrawGridItem(dsurface, Bounds(DDGrid.SurfaceX(Rect.Left), DDGrid.SurfaceY(Rect.Top), DDGrid.ColWidth, DDGrid.RowHeight), @g_DealItems[nIdx], @g_DealItemsEffect[nIdx]);
  end;
end;
{var
  nIdx: Integer;
...
begin
  ...
  if nIdx in [0..9] then begin
    if g_DealItems[nIdx].s.Name <> '' then begin
      ...
      with DDGrid do begin
        if (g_DealItems[nIdx].s.AddValue[12] in [1, 3])  then begin
...
begin
  ...
  if nIdx in [0..9] then begin
    DrawGridItem(dsurface, Bounds(DDRGrid.SurfaceX(Rect.Left), DDRGrid.SurfaceY(Rect.Top), DDRGrid.ColWidth, DDRGrid.RowHeight), @g_DealRemoteItems[nIdx], @g_DealRemoteItemsEffect[nIdx]);
  end;
end;
{var
  nIdx: Integer;
...
begin
  ...
  if nIdx in [0..19] then begin
    if g_DealRemoteItems[nIdx].s.Name <> '' then begin
      ...
      with DDRGrid do begin
        if (g_DealRemoteItems[nIdx].s.AddValue[12] in [1, 3]) then begin
  ...
  with DUserState1 do begin
   ...
   // idx := 440 + hair div 2;
    ...
    if UserState1.UseItems[U_DRESS].s.Name <> '' then begin
      ...
      DrawBodyItemEffect(dsurface, SurfaceX(bbx), SurfaceY(bby), @UserState1.UseItems[U_DRESS].s, @g_UseItemsEffect1[U_DRESS]);
    end;
    if UserState1.UseItems[U_WEAPON].s.Name <> '' then begin
      ...
      if Idx >= 0 then begin
        ...
        if d <> nil then
          dsurface.Draw(SurfaceX(bbx + ax), SurfaceY(bby + ay), d.ClientRect, d, True);
        DrawBodyItemEffect(dsurface, SurfaceX(bbx), SurfaceY(bby), @UserState1.UseItems[U_WEAPON].s, @g_UseItemsEffect1[U_WEAPON]);
    ...
    if UserState1.UseItems[U_HELMET].s.Name <> '' then begin
      ...
      DrawBodyItemEffect(dsurface, DHelmetUS1.VirtualRect, @UserState1.UseItems[U_HELMET].s, @g_UseItemsEffect1[U_HELMET]);
    end;
...
begin
  ...
  if nWhere >= 0 then begin
    if (nWhere in [U_DRESS, U_WEAPON]) then
      DrawGridItem(dsurface, TDButton(Sender).VirtualRect, @UserState1.UseItems[nWhere].s, @g_UseItemsEffect1[nWhere])
    else
      DrawBodyItem(dsurface, TDButton(Sender).VirtualRect, @UserState1.UseItems[nWhere].s, @g_UseItemsEffect1[nWhere]);
  end;
  ...
  if nWhere >= 0 then begin
    if UserState1.UseItems[nWhere].s.Name <> '' then begin
      ...
      if Idx >= 0 then begin
        ...
        if (UserState1.UseItems[nWhere].s.AddValue[12] in [1, 3]) then begin
  ...
  if g_ConfigClient.btMainInterface in [0, 2] then begin
    if (X >= DBottom.Left + 666) and (Y >= SCREENHEIGHT - 73) and
      ...
      if g_MySelf.m_Abil.MaxExp > 0 then begin
        sStr := '当前经验' + FloatToStrFixFmt(g_MySelf.m_Abil.Exp / g_MySelf.m_Abil.MaxExp * 100, 3, 2) + '%\';
        //sStr := '当前经验' + Format('%f',[100 * g_MySelf.m_Abil.Exp / g_MySelf.m_Abil.MaxExp])+ '%\';
    ...
    if (Y >= SCREENHEIGHT - 8) and (Y <= SCREENHEIGHT) then begin
      ...
      if g_MySelf.m_Abil.MaxExp > 0 then begin
        sStr := '当前经验' + FloatToStrFixFmt(g_MySelf.m_Abil.Exp / g_MySelf.m_Abil.MaxExp * 100, 3, 2) + '%\';
        // sStr := sStr+'当前经验' + Format('%f',[100 * g_MySelf.m_Abil.Exp / g_MySelf.m_Abil.MaxExp])+ '%\';
      ...
      if Idx > 0 then begin
        ...
        if d <> nil then begin
          if g_MyHero.m_boDeath then begin
            ...
            DrawEffect(g_ImgMixSurface, ceGrayScale);
            d := g_ImgMixSurface;
          end;
    ...
    case HeroStatePage of
      0: begin
          ...
          if g_HeroUseItems[U_DRESS].s.Name <> '' then begin
            ...
            DrawBodyItemEffect(dsurface, SurfaceX(bbx), SurfaceY(bby), @g_HeroUseItems[U_DRESS].s, @g_HeroUseItemsEffect[U_DRESS]);
          end;
          if g_HeroUseItems[U_WEAPON].s.Name <> '' then begin
            ...
            if Idx >= 0 then begin
              ...
              DrawBodyItemEffect(dsurface, SurfaceX(bbx), SurfaceY(bby), @g_HeroUseItems[U_WEAPON].s, @g_HeroUseItemsEffect[U_WEAPON]);
          ...
          if g_HeroUseItems[U_HELMET].s.Name <> '' then begin
            ...
            if Idx >= 0 then begin
              ...
              if d <> nil then
                dsurface.Draw(SurfaceX(bbx + ax), SurfaceY(bby + ay), d.ClientRect, d, True);
            //DrawBodyItemEffect(dsurface, SurfaceX(bbx), SurfaceY(bby), @g_UseItems[U_HELMET].s, @g_UseItemsEffect[U_HELMET]);
              DrawBodyItemEffect(dsurface, DHeroSWHelmet.VirtualRect, @g_HeroUseItems[U_HELMET].s, @g_HeroUseItemsEffect[U_HELMET]);
            end;
          end;
        end;
      1: begin
...
begin
  ...
  if HeroStatePage = 0 then begin
    ...
    if nWhere >= 0 then begin
      if (nWhere in [U_DRESS, U_WEAPON]) then
        DrawGridItem(dsurface, TDButton(Sender).VirtualRect, @g_HeroUseItems[nWhere].s, @g_HeroUseItemsEffect[nWhere])
      else
        DrawBodyItem(dsurface, TDButton(Sender).VirtualRect, @g_HeroUseItems[nWhere].s, @g_HeroUseItemsEffect[nWhere]);
    end;
    ...
    if nWhere >= 0 then begin
      if g_HeroUseItems[nWhere].s.Name <> '' then begin
        ...
        if Idx >= 0 then begin
          ...
          if (g_HeroUseItems[nWhere].s.AddValue[12] in [1, 3])  then begin
...
begin
  ...
  if nIdx in [Low(THeroItemArr)..High(THeroItemArr)] then begin
    if not g_boItemMoving then begin
      ClearMoveRect();
...
begin
  ...
  nIdx := ACol + ARow * DHeroItemGrid.ColCount;
  if nIdx in [Low(THeroItemArr)..High(THeroItemArr)] then begin
    DrawGridItem(dsurface, Bounds(DHeroItemGrid.SurfaceX(Rect.Left), DHeroItemGrid.SurfaceY(Rect.Top), DHeroItemGrid.ColWidth, DHeroItemGrid.RowHeight), @g_HeroItemArr[nIdx], @g_HeroItemArrEffect[nIdx]);
  end;
end;
{var
  nIdx: Integer;
...
begin
  ...
  if nIdx in [Low(THeroItemArr)..High(THeroItemArr)] then begin
    if g_HeroItemArr[nIdx].s.Name <> '' then begin
      ...
      with DHeroItemGrid do begin
        if (g_HeroItemArr[nIdx].s.AddValue[12] in [1, 3]) then begin
...
begin
  ...
  if ssRight in Shift then begin
    ...
    nIdx := ACol + ARow * DHeroItemGrid.ColCount;
    if (nIdx in [Low(g_HeroItemArr)..High(g_HeroItemArr)]) then begin
...
procedure TFrmDlg.DUpgradeItem1DirectPaint(Sender: TObject;
  dsurface: TTexture);
begin
  DrawGridItem(dsurface, TDButton(Sender).VirtualRect, @g_UpgradeItems[TDButton(Sender).Tag], @g_UpgradeItemsEffect[TDButton(Sender).Tag]);
end;
{var
  Idx: Integer;
  d: TTexture;
...
begin
  ...
  if nIdx in [0..4] then begin
    DrawGridItem(dsurface, Bounds(DDRDuelGrid.SurfaceX(Rect.Left), DDRDuelGrid.SurfaceY(Rect.Top), DDRDuelGrid.ColWidth, DDRDuelGrid.RowHeight), @g_DuelRemoteItems[nIdx], @g_DuelRemoteItemsEffect[nIdx]);
  end;
end;
{var
  nIdx: Integer;
...
begin
  ...
  if nIdx in [0..4] then begin
    if g_DuelRemoteItems[nIdx].s.Name <> '' then begin
      ...
      with DDRDuelGrid do begin
        if (g_DuelRemoteItems[nIdx].s.AddValue[12] in [1, 3]) then begin
...
begin
  ...
  if nIdx in [0..4] then begin
    DrawGridItem(dsurface, Bounds(DDDuelGrid.SurfaceX(Rect.Left), DDDuelGrid.SurfaceY(Rect.Top), DDDuelGrid.ColWidth, DDDuelGrid.RowHeight), @g_DuelItems[nIdx], @g_DuelItemsEffect[nIdx]);
  end;
end;
{var
  nIdx: Integer;
...
begin
  ...
  if nIdx in [0..4] then begin
    if g_DuelItems[nIdx].s.Name <> '' then begin
      ...
      with DDDuelGrid do begin
        if (g_DuelItems[nIdx].s.AddValue[12] in [1, 3]) then begin
...
begin
  ...
  if nIdx in [0..10 - 1] then begin
    if g_ShopItems[ShopTabPage, nIdx].StdItem.Name <> '' then begin
      DrawGridStdItem(dsurface, Bounds(DShopItemGrid.SurfaceX(Rect.Left) + 10, DShopItemGrid.SurfaceY(Rect.Top), 32, DShopItemGrid.RowHeight), @g_ShopItems[ShopTabPage, nIdx].StdItem, @g_ShopItemsEffect[ShopTabPage, nIdx]);
      ...
      if d <> nil then
        with DShopItemGrid do begin
          dsurface.Draw(SurfaceX(Rect.Left + 10 + (32 - d.Width) div 2 - 1),
            SurfaceY(Rect.Top + (RowHeight - d.Height) div 2 + 1),       SurfaceX(Rect.Left), SurfaceY(Rect.Top)
            d.ClientRect,
            d, True);
        end; }
...
begin
  ...
  if nIdx in [0..5 - 1] then begin
    if g_SuperShopItems[nIdx].StdItem.Name <> '' then begin
      DrawGridStdItem(dsurface, Bounds(DSuperShopItemGrid.SurfaceX(Rect.Left) + 39, DSuperShopItemGrid.SurfaceY(Rect.Top), 42, 32), @g_SuperShopItems[nIdx].StdItem, @g_SuperShopItemsEffect[nIdx]);
      {d := g_WBagItemImages.Images[g_SuperShopItems[nIdx].StdItem.looks];
      if d <> nil then
        with DSuperShopItemGrid do begin                                            DSuperShopItemGrid.RowHeight
  ...
  if g_ConfigClient.btMainInterface in [0, 2] then begin
    if g_MyHero.m_Abil.MaxExp > 0 then begin
      sStr := '体力值: ' + IntToStr(g_MyHero.m_Abil.HP) + '/' + IntToStr(g_MyHero.m_Abil.MaxHP) + '\' +
        ...
        '经验值: ' + FloatToStrFixFmt(g_MyHero.m_Abil.Exp / g_MyHero.m_Abil.MaxExp * 100, 3, 2) + '%\';
...
begin
  ...
  if nIdx in [0..14] then begin
    DrawGridItem(dsurface, Bounds(DGStore.SurfaceX(Rect.Left), DGStore.SurfaceY(Rect.Top), DGStore.ColWidth, DGStore.RowHeight), @g_StoreItems[nIdx].Item, @g_StoreItemsEffect[nIdx]);
  end;
end;
{var
  nIdx: Integer;
...
begin
  ...
  if nIdx in [0..15 - 1] then begin
    if g_StoreItems[nIdx].Item.s.Name <> '' then begin
      ...
      with DGStore do begin
        if (g_StoreItems[nIdx].Item.s.AddValue[12] in [1, 3]) then begin
...
begin
  ...
  if nIdx in [0..14] then begin
    DrawGridItem(dsurface, Bounds(DGUserStore.SurfaceX(Rect.Left), DGUserStore.SurfaceY(Rect.Top), DGUserStore.ColWidth, DGUserStore.RowHeight), @g_StoreRemoteItems[nIdx].Item, @g_StoreRemoteItemsEffect[nIdx]);
  end;
  ...
  if nIdx in [0..14] then begin
    if g_StoreRemoteItems[nIdx].Item.s.Name <> '' then begin
     ...
     { with DGUserStore do begin
        if (g_StoreRemoteItems[nIdx].Item.s.AddValue[12] in [1, 3])  then begin
...
begin
  ...
  case nIndex of
    ...
    1: begin
        ...
        if Assigned(g_PlugInfo.MediaPlayer.Player) then begin
          g_PlugInfo.MediaPlayer.Player(PAnsiChar(Trim(sItemData)), False, True);
        end;
    ...
    2: begin
        ...
        if Assigned(g_PlugInfo.OpenHomePage) then begin
          g_PlugInfo.OpenHomePage(PAnsiChar(Trim(sItemData)));
        end;
      end;
  end;
end;
...
procedure TFrmDlg.DrawGridItem(dsurface: TTexture; ARect: TRect; Item: PTClientItem; Effect: Pointer);
var
  d: TTexture;
...
begin
  if (Item <> nil) and (Item.s.Name <> '') then begin
    ...
    if d <> nil then
      dsurface.Draw(ARect.Left + ((ARect.Right - ARect.Left) - d.Width) div 2 - 1,
        ARect.Top + ((ARect.Bottom - ARect.Top) - d.Height) div 2 + 1,
        d.ClientRect,
        d);

    if (Effect <> nil) and (Item.s.EffectFileIndex >= 0) and
      (Item.s.EffectFileIndex < g_ServerEffectImageList.Count) and
      (Item.s.EffectImageCount > 0) then begin
      dwTime := _MAX(100 div (_MAX(Item.s.EffectImageCount, 10) div 10), 10);
      if GetTickCount - DrawEffect.dwDrawTick >= dwTime then begin
        DrawEffect.dwDrawTick := GetTickCount;
        Inc(DrawEffect.btDrawCount);
      end;

      if DrawEffect.btDrawCount >= Item.s.EffectImageCount then
        DrawEffect.btDrawCount := 0;

      GameImages := TGameImages(g_ServerEffectImageList.Objects[Item.s.EffectFileIndex]);
      if GameImages <> nil then begin
        d := GameImages.Images[Item.s.EffectStartOffSet + DrawEffect.btDrawCount];
        if d <> nil then
          DrawBlend(dsurface, ARect.Left + ((ARect.Right - ARect.Left) - d.Width) div 2 - 1,
            ARect.Top + ((ARect.Bottom - ARect.Top) - d.Height) div 2 + 1,
...
procedure TFrmDlg.DrawGridStdItem(dsurface: TTexture; ARect: TRect; Item: pTStdItem; Effect: Pointer);
var
  d: TTexture;
  DrawEffect: pTDrawItemEffect;
  GameImages: TGameImages;
  ...
  if (Item <> nil) and (Item.Name <> '') then begin
    ...
    if d <> nil then
      dsurface.Draw(ARect.Left + ((ARect.Right - ARect.Left) - d.Width) div 2 - 1,
        ...
        d.ClientRect,
        d);
    //DScreen.AddChatBoardString('Item.EffectFileIndex:' + IntToStr(Item.EffectFileIndex) + ' Item.EffectImageCount:' + IntToStr(Item.EffectImageCount), clyellow, clRed);
    if (Effect <> nil) and (Item.EffectFileIndex >= 0) and
      (Item.EffectFileIndex < g_ServerEffectImageList.Count) and
      (Item.EffectImageCount > 0) then begin
      dwTime := _MAX(100 div (_MAX(Item.EffectImageCount, 10) div 10), 10);
      if GetTickCount - DrawEffect.dwDrawTick >= dwTime then begin
        DrawEffect.dwDrawTick := GetTickCount;
        Inc(DrawEffect.btDrawCount);
      end;

      if DrawEffect.btDrawCount >= Item.EffectImageCount then
        DrawEffect.btDrawCount := 0;

      GameImages := TGameImages(g_ServerEffectImageList.Objects[Item.EffectFileIndex]);
      if GameImages <> nil then begin
        d := GameImages.Images[Item.EffectStartOffSet + DrawEffect.btDrawCount];
        if d <> nil then
          DrawBlend(dsurface, ARect.Left + ((ARect.Right - ARect.Left) - d.Width) div 2 - 1,
            ARect.Top + ((ARect.Bottom - ARect.Top) - d.Height) div 2 + 1,
            d);
...
procedure TFrmDlg.DrawBodyItem(dsurface: TTexture; ARect: TRect; Item: pTStdItem; Effect: Pointer);
var
  d: TTexture;
  DrawEffect: pTDrawItemEffect;
  GameImages: TGameImages;
...
begin
  if (Item <> nil) and (Item.Name <> '') then begin
    ...
    d := GetWStateImg(Item.looks);
    if d <> nil then
      dsurface.Draw(ARect.Left + ((ARect.Right - ARect.Left) - d.Width) div 2 - 1,
        ARect.Top + ((ARect.Bottom - ARect.Top) - d.Height) div 2 + 1,
        d.ClientRect,
        d);
    if (Effect <> nil) and (Item.BodyEffectFileIndex >= 0) and
      (Item.BodyEffectFileIndex < g_ServerEffectImageList.Count) and
      (Item.BodyEffectImageCount > 0) then begin

      dwTime := _MAX(100 div (_MAX(Item.BodyEffectImageCount, 10) div 10), 10);
      if GetTickCount - DrawEffect.dwDrawTick >= dwTime then begin
        DrawEffect.dwDrawTick := GetTickCount;
        Inc(DrawEffect.btDrawCount);
      end;

      if DrawEffect.btDrawCount >= Item.BodyEffectImageCount then
        DrawEffect.btDrawCount := 0;

      GameImages := TGameImages(g_ServerEffectImageList.Objects[Item.BodyEffectFileIndex]);
      if GameImages <> nil then begin
        d := GameImages.GetCachedImage(Item.BodyEffectStartOffSet + DrawEffect.btDrawCount, nX, nY);
        if d <> nil then
          DrawBlend(dsurface, ARect.Left + ((ARect.Right - ARect.Left) - d.Width) div 2 + nX - 1,
            ARect.Top + ((ARect.Bottom - ARect.Top) - d.Height) div 2 + nY + 1,
            d);
      end;
    end;
  end;
...
procedure TFrmDlg.DrawBodyItemEffect(dsurface: TTexture; ARect: TRect; Item: pTStdItem; Effect: Pointer); //画出装备特效
var
  d: TTexture;
  DrawEffect: pTDrawItemEffect;
  GameImages: TGameImages;
...
procedure TFrmDlg.DrawBodyItemEffect(dsurface: TTexture; X, Y: Integer; Item: pTStdItem; Effect: Pointer);
var
  d: TTexture;
  DrawEffect: pTDrawItemEffect;
  GameImages: TGameImages;
  ...
  if (Item <> nil) and (Item.Name <> '') then begin
    ...
    if (Effect <> nil) and (Item.BodyEffectFileIndex >= 0) and
      ...
      if GameImages <> nil then begin
        d := GameImages.GetCachedImage(Item.BodyEffectStartOffSet + DrawEffect.btDrawCount, nX, nY);
        if d <> nil then
          DrawBlend(dsurface, X + nX - 1, Y + nY, d);
      end;
      {except
        DScreen.AddChatBoardString('g_ServerEffectImageList:'+g_ServerEffectImageList.Strings[Item.BodyEffectFileIndex], clGreen, clWhite);
      end;}
    end;
  end;
end;
