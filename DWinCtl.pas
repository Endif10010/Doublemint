...
type
  ...
  public
    ...
    procedure FastSetParent(Value: TDControl);
  ...
  private
    ...
    FCaptionSurface: pTFontText;
    FDrawCaptionTick: LongWord;
...
procedure TDControl.FastSetParent(Value: TDControl);
begin
  Parent := Value;
  FDParent := Value;
...
begin
  ...
  if Caption <> '' then begin
    if Caption = '-' then begin
      ...
      if FBold then begin
        if (FCaptionSurface = nil) or (GetTickCount - FDrawCaptionTick > 10000) then begin
          dsurface.BoldTextOut(nX,
            SurfaceY(Top) + (Height - dsurface.TextHeight('0')) div 2, Caption, FCurrColor, FBoldColor);
          FDrawCaptionTick := GetTickCount;
          FCaptionSurface := ImageFont.GetFontText(Caption, FCurrColor, FBoldColor);
          //showmessage('TDLabel.DirectPaint:'+Caption);
          ...
          FDrawCaptionTick := GetTickCount;
          FCaptionSurface.Time := GetTickCount;

          dsurface.Draw(nX,
            SurfaceY(Top) + (Height - dsurface.TextHeight('0')) div 2, FCaptionSurface.Font.ClientRect, FCaptionSurface.Font);
        ...
        if (FCaptionSurface = nil) or (GetTickCount - FDrawCaptionTick > 10000) then begin
          ...
          FDrawCaptionTick := GetTickCount;
          FCaptionSurface := ImageFont.GetFontText(Caption, FCurrColor);
          //dsurface.TextOut(450,420,'FCaptionSurface = nil');
        end else begin
          //dsurface.TextOut(450,450,'FCaptionSurface <> nil');
          FDrawCaptionTick := GetTickCount;
          FCaptionSurface.Time := GetTickCount;
