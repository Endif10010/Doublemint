...
type
  ...
  public
    ...
    function GetTextDIB(Text: string; FColor: TColor; BColor: TColor = clBlack): TTexture;
    function GetFontText(Text: string; FColor: TColor; BColor: TColor = clBlack): pTFontText;
    property Count: Integer read GetCount;
  end;
  ...
  if Source <> nil then begin
    ...
    if ClipRect2(DestRect, SrcRect, ClientRect, Source.ClientRect) then begin
      ...
      if TRANSPARENT then begin
        ...
        for nY := 0 to nHeight - 1 do begin
          nSrcH := Min(srcy_16 shr 16 + SrcRect.Top, Source.Height - 1);
          nDstH := Min(DestRect.Top + nY, DIB.Height - 1);
          pSrc := PByte(Integer(Source.PBits) + nSrcH * Source.Pitch + nSLeft);
          pDst := PByte(Integer(DIB.PBits) + nDstH * DIB.Pitch + nDLeft);
          srcx_16 := 0;
          for nX := 0 to nWidth - 1 do begin
            nSrcW := Min(srcx_16 shr 16, Source.Width - SrcRect.Left - 1);
            PWordArray(pDst)[nX] := PWordArray(pSrc)[nSrcW];
        ...
        for nY := 0 to nHeight - 1 do begin
          nSrcH := Min(srcy_16 shr 16 + SrcRect.Top, Source.Height - 1);
          nDstH := Min(DestRect.Top + nY, Height - 1);
          pSrc := PByte(Integer(Source.PBits) + nSrcH * Source.Pitch + nSLeft);
          pDst := PByte(Integer(PBits) + nDstH * Pitch + nDLeft);
          srcx_16 := 0;
          for nX := 0 to nWidth - 1 do begin
            nSrcW := Min(srcx_16 shr 16, Source.Width - SrcRect.Left - 1);
            PWordArray(pDst)[nX] := PWordArray(pSrc)[nSrcW];
    ...
    if (Source.Width = (Dest.Right - Dest.Left)) and
      ...
      if ClipRect(DestRect, ClientRect) then begin
        ...
        if TRANSPARENT then begin
          ...
          for nY := 0 to nHeight - 1 do begin
            nSrcH := Min(srcy_16 shr 16, Source.Height - 1);
            nDstH := Min(nY, DIB.Height - 1);
            pSrc := PByte(Integer(Source.PBits) + nSrcH * Source.Pitch);
            pDst := PByte(Integer(DIB.PBits) + nDstH * DIB.Pitch);
            srcx_16 := 0;
            for nX := 0 to nWidth - 1 do begin
              nSrcW := Min(srcx_16 shr 16, Source.Width - 1);
              PWordArray(pDst)[nX] := PWordArray(pSrc)[nSrcW];
              Inc(srcx_16, xrIntFloat_16);
            end;
            Inc(srcy_16, yrIntFloat_16);
          ...
          for nY := 0 to nHeight - 1 do begin
            ...
            nDstH := Min(DestRect.Top + nY, Height - 1);
            pSrc := PByte(Integer(Source.PBits) + nSrcH * Source.Pitch);
            pDst := PByte(Integer(PBits) + nDstH * Pitch + nLeft);
...
begin
  if (GetTickCount - FreeOutTimeTick > 1000 * 10) then begin
    ...
    for I := FontList.Count - 1 downto 0 do begin
      TextureFont := FontList.Items[I];
      if (GetTickCount - TextureFont.Time > 1000 * 60 * 2) then begin
        FontList.Delete(I);
        try
          FreeMem(TextureFont.Data);
        except
          TextOutFile('FreeOutTime1');
        end;
        try
          Dispose(TextureFont);
        except
          TextOutFile('FreeOutTime2');
        end;
      end;
    end;
  end;
...
function TImageFont.GetFontText(Text: string; FColor: TColor; BColor: TColor): pTFontText;
var
  I: Integer;
  FontText: pTFontText;
...
begin
  ...
  //if BColor = clBlack then BColor := $00050505;
  for I := 0 to FontTextList.Count - 1 do begin
    FontText := FontTextList.Items[I];
...
function TImageFont.GetTextDIB(Text: string; FColor: TColor; BColor: TColor): TTexture;
var
  I: Integer;
  FontText: pTFontText;
  FontStyle: TFontStyles;
