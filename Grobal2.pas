...
interface
uses
  Windows, Classes, D7ScktComp, Controls;
const
...
{  BLUE_READ_898 = 5165; //获取英雄忠诚  10001(忠00.00%)
  ...
  CM_MOVEITEMVALUE = 5219;

  SM_STARTMOVEITEMVALUE = 5220; //开始转移属性
  SM_STOPMOVEITEMVALUE = 5221; //停止转移属性

  SM_MOVEITEMVALUE_OK = 5222; //转移属性 成功
  SM_MOVEITEMVALUE_FAIL = 5223; //转移属性  失败

  CM_STOPMOVEITEMVALUE = 5224; //停止转移属性


  SM_SERVEREFFECTIMAGELIST = 5225;
  SM_HUMEFFECT = 5226;
  SM_CHANGESPEED = 5227; //游戏速度
  SM_PLAYEFFECT = 5228;
  SM_SCREENEFFECT = 5229;
  SM_SENDUSEREFFECT = 5230;
  ...
  RM_HEAR2 = 30040;
  RM_HUMEFFECT = 30041;
  RM_CHANGESPEED = 30042;
  RM_PLAYEFFECT = 30043;
  RM_SCREENEFFECT = 30044;
  RM_SENDUSEREFFECT = 30045;
...
type
  ...
  TItemEffect = record
    ...
    FileIndex2: Integer; //包裹
    FileIndex3: Integer; //外观
    OffSet1: Integer; //内观
    OffSet2: Integer; //包裹
    OffSet3: Integer; //外观
    ImageCount1: Integer; //内观
    ImageCount2: Integer; //包裹
    EffectDesc: string;
  end;
  pTItemEffect = ^TItemEffect;

  THumEffect = record //size 23
//装备特效
    nWeaponEffectIndex: SmallInt;
    wWeaponEffectOffSet: Word;
    nDressEffectIndex: SmallInt;
    wDressEffectOffSet: Word;
  end;
  pTHumEffect = ^THumEffect;
...
{$IF HEROVERSION = 1}
    ...
    MaxDate: TDateTime;
    //Reserveds: array[0..20 - 1] of Integer;
{$IFEND}
  end;
  ...
  THumAddItems_2012_09_15_qian = array[9..12] of TUserItem;
...
{$IFEND}
 ...
 { TUserLevelSort = record //人物等级排行
  ...
  {TUsrEngnLicense = record
    ...
    nQQCode: Integer;
    Handle3: Integer;
    sSerialNumber: array[0..32 - 1] of AnsiChar;
    Handle4: Integer;
    btMode: Byte;
    Handle5: Integer;
    btSoftType: Byte;
...
function TouLong(x: PAnsiChar): LongWord;
