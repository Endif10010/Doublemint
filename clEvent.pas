...
begin
  if (not CanDraw) or (not m_boVisible) then Exit;
...
begin
  if m_boVisible and (GetTickCount - m_dwFrameTime > m_nRunTime) then begin
    m_dwFrameTime := GetTickCount;
    Inc(m_dwCurframe);
  end;
end;
...
{7种烟花效果}
    ...
    if m_nExplosionSound > 0 then begin
      PlaySound(m_nExplosionSound);
      m_nExplosionSound := -2;
    end;
  end;
  inherited;
...
begin
  for I := 0 to EventList.Count - 1 do
    if EventList[I] = evn then begin
      TEvent(EventList[I]).m_boVisible := False;
      Break;
    end;
end;
...
begin
  Result := nil;
  for I := 0 to EventList.Count - 1 do
    if TEvent(EventList[I]).m_boVisible and (TEvent(EventList[I]).m_nX = ax) and (TEvent(EventList[I]).m_nY = ay) and
