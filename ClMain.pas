...
uses
  ...
  D7ScktComp, ExtCtrls, Textures, DXDraws, DirectX, DXClass, DrawScrn, IntroScn, PlayScn,
...
type
  TLoadShowItemThread = class(TThread)
  protected
  ...
  TCheckHackThread = class(TThread)
    m_dwRunTick: LongWord;
    m_dwCheckProcessesTick: LongWord;
    m_dwCheckWindowTitleTick: LongWord;
  private
    procedure Run;
  protected
    procedure Execute; override;
  ...
  TfrmMain = class(TDxForm)
    ...
    procedure HTTPGetTitleListDoneString(Sender: TObject; Result: string);
    procedure CSocketConnecting(Sender: TObject; Socket: TCustomWinSocket);
    procedure HTTPGetTitleListError(Sender: TObject);
    procedure HTTPGetCheckProError(Sender: TObject);
    procedure TimerShowDlgTimer(Sender: TObject);
  private
    ...
    procedure ClientGetServerEffectImageList(body: string);
    procedure ClientGetHumEffect(DefMsg: pTDefaultMessage; body: string);
    procedure ClientGetUseEffects(DefMsg: pTDefaultMessage; sBody: string);
    procedure ClientObjScreenEffect(DefMsg: pTDefaultMessage; sData: string);
  ...
  public
    ...
    procedure SendMoveItemValue(nSourceItemIdx, nDestItemIdx: Integer);
...
var
  ShowMsgActor: TActor;
  AppHandle: HWnd;
  CheckHackThread: TCheckHackThread = nil;
...
begin
  ...
  case wIdent of
    ...
    GL_FILTERMODULE: begin
        CSocket.Active := False;
        SpeedHackTimer.Enabled := False;
        FrmDlg.DMessageDlg('(3)速度异常，游戏已被中止。 \' +
          '如有问题请联系游戏管理员。',
          [mbOk]);
      end;
  end;
...
begin
  ...
  if Msg.Message = WM_KeyDown then begin
    ...
    if Msg.wParam = VK_TAB then begin
      if (g_FocusCret <> nil) and PlayScene.IsValidActor(g_FocusCret) and (g_FocusCret.m_btHair < 4) then begin
...
begin
  ...
  g_ConfigClient.btMainInterface := 0; 
  FInterval2 := 1;
  FInterval := 30;
  FOldTime := TimeGetTime;
  FOldTime2 := TimeGetTime;

  BugTextOut := DebugOutStr;
...
{$ELSE}
  g_boFullScreen := False; //61.160.247.208
  //g_sServerAddr := '115.239.229.224'; // //'121.14.151.47'; '127.0.0.1'; //; //;'121.14.152.5'; //'121.11.91.39'; // '127.0.0.1'; //;////117.41.165.47 7001
  //g_nServerPort := 7000;

  g_sServerAddr := '127.0.0.1'; //  118.123.20.74
  g_nServerPort := 7000;
...
{$IFEND}
  ...
  CheckHackThread := TCheckHackThread.Create;
  ...
  g_CreateEffectImageList := TStringList.Create;
  g_ServerEffectImageList := TStringList.Create;
  g_ClientEffectImageList := TStringList.Create;
...
{$ELSE}
  ...
  g_ConfigClient.btItemColor := 255;
  LoadItemDescList;
  try
    if FileExists('FilterItemList.txt') then
    g_ShowItemFileList.LoadFromFile('FilterItemList.txt');
  except
...
{$IFEND}
 ...
 // Caption := g_sLogoText;
  ...
  g_WindowsTitleList := TList.Create;
  ...
  FillChar(g_ShopItems, SizeOf(g_ShopItems), #0); //商铺物品
  FillChar(g_SuperShopItems, SizeOf(g_SuperShopItems), #0); //商铺物品

  FillChar(g_DrawUseItems, SizeOf(TDrawItemEffect) * 13, #0);
  FillChar(g_DrawUseItems1, SizeOf(TDrawItemEffect) * 13, #0);
  FillChar(g_DrawHeroUseItems, SizeOf(TDrawItemEffect) * 13, #0);

  FillChar(g_DrawItemArr, SizeOf(TDrawItemEffect) * 46, #0);
  FillChar(g_DrawHeroItemArr, SizeOf(TDrawItemEffect) * 40, #0);
  FillChar(g_DrawDealRemoteItems, SizeOf(TDrawItemEffect) * 20, #0);
  FillChar(g_DrawDealItems, SizeOf(TDrawItemEffect) * 20, #0);


  FillChar(g_DrawUseItems_, SizeOf(TDrawItemEffect) * 13, #0);
  FillChar(g_DrawUseItems1_, SizeOf(TDrawItemEffect) * 13, #0);
  FillChar(g_DrawHeroUseItems_, SizeOf(TDrawItemEffect) * 13, #0);

  FillChar(g_DrawItemArr_, SizeOf(TDrawItemEffect) * 46, #0);
  FillChar(g_DrawHeroItemArr_, SizeOf(TDrawItemEffect) * 40, #0);
  FillChar(g_DrawDealRemoteItems_, SizeOf(TDrawItemEffect) * 20, #0);
  FillChar(g_DrawDealItems_, SizeOf(TDrawItemEffect) * 20, #0);


  FillChar(g_StoreItems, SizeOf(TStoreItem) * 15, #0);
  FillChar(g_StoreRemoteItems, SizeOf(TStoreItem) * 15, #0);
  FillChar(g_DrawStoreItems, SizeOf(TDrawItemEffect) * 15, #0);
  FillChar(g_DrawStoreItems_, SizeOf(TDrawItemEffect) * 15, #0);
  FillChar(g_DrawStoreRemoteItems, SizeOf(TDrawItemEffect) * 15, #0);
  FillChar(g_DrawStoreRemoteItems_, SizeOf(TDrawItemEffect) * 15, #0);

  for I := 0 to Length(g_DrawStarsTick) - 1 do begin
  ...
  FillChar(g_ItemArrEffect, SizeOf(g_ItemArrEffect), #0);
  FillChar(g_HeroItemArrEffect, SizeOf(g_HeroItemArrEffect), #0);
  FillChar(g_UseItemsEffect, SizeOf(g_UseItemsEffect), #0);
  FillChar(g_HeroUseItemsEffect, SizeOf(g_HeroUseItemsEffect), #0);
  FillChar(g_UseItemsEffect1, SizeOf(g_UseItemsEffect1), #0);
  FillChar(g_DealItemsEffect, SizeOf(g_DealItemsEffect), #0);
  FillChar(g_DealRemoteItemsEffect, SizeOf(g_DealRemoteItemsEffect), #0);

  FillChar(g_DuelItemsEffect, SizeOf(g_DuelItemsEffect), #0);
  FillChar(g_DuelRemoteItemsEffect, SizeOf(g_DuelRemoteItemsEffect), #0);

  FillChar(g_StoreItemsEffect, SizeOf(g_StoreItemsEffect), #0);
  FillChar(g_StoreRemoteItemsEffect, SizeOf(g_StoreRemoteItemsEffect), #0);

  FillChar(g_ShopItemsEffect, SizeOf(g_ShopItemsEffect), #0); //商铺物品
  FillChar(g_SuperShopItemsEffect, SizeOf(g_SuperShopItemsEffect), #0); //商铺物品

  FillChar(g_UpgradeItemsEffect, SizeOf(g_UpgradeItemsEffect), #0);
  ...
  g_WaitingMoveItemValue.Item.S.Name := '';
  g_EatingItem.S.Name := '';
...
procedure TLoadShowItemThread.Execute;
var
  I, nTop: Integer;
  DLabel: TDLabel;
...
begin
  ...
  with FrmDlg do begin
    ...
    for I := 0 to List.Count - 1 do begin
      if Application.Terminated or Terminated then break;
      ...
      //DLabel.FastSetParent(DMemoTab2);
     ...
     // ItemList.AddObject(DLabel.Caption, DLabel);
      DMemoTab2.AddSuItem(ItemList, DLabel);

      DCheckBox := TDCheckBox.Create(DMemoTab2);
        //DLabel.DParent := DMemoTab2;
      DCheckBox.FastSetParent(DMemoTab2);
      ...
      ItemList.AddObject(DCheckBox.Caption, DCheckBox);
    ...
    DMemoTab2.RefreshPos;
    DScrollTab2.Max := DMemoTab2.MaxHeight;
  end;
  List.Free;
end;

constructor TLoadShowItemThread.Create();
begin
...
begin
  g_PlugInfo.AppHandle := Handle;
  g_MainHandle := Handle;
  SendGameCenterMsg(CM_Initialize, IntToStr(Handle));
      ...
      if g_boItemMoving then begin
        ...
        //iamwgh 画移动物品跟随鼠标，此处修正坐标以适应非全屏状态位置
          ImageCanvas.Draw(p.X - (d.ClientRect.Right div 2),
            ...
            d.ClientRect,
            d);
          if g_boStartMoveItemValue and (g_MovingItem.Item.S.Name <> g_sGoldName {'金币'}) then begin
            if (g_MouseItem.s.Name <> '') then begin
              sText := Format('鼠标点一下“%s”，把“%s”的属性转移到该装备', [g_MouseItem.s.Name, g_MovingItem.Item.S.Name]);
              FC := clYellow;
              BC := clRed;
            end else begin
              sText := Format('拖动“%s”，放到目标装备上进行属性转移', [g_MovingItem.Item.S.Name]);
              FC := clLime;
              BC := $00050505;
            end;
            MainForm.Canvas.Font.Size := 16;
            MainForm.Canvas.Font.Name := g_sFontName;

            nX := p.X - ImageCanvas.TextWidth(sText) div 2;
            if nX < 0 then nX := 0;
            if nX + ImageCanvas.TextWidth(sText) > SCREENWIDTH then
              nX := SCREENWIDTH - ImageCanvas.TextWidth(sText);
            nY := p.Y - d.Height - 16;
            if nY < 0 then nY := 0;

            if nY + ImageCanvas.TextHEIGHT(sText) > SCREENHEIGHT then
              nY := SCREENHEIGHT - ImageCanvas.TextHEIGHT(sText);

            ImageCanvas.BoldTextOut(nX, nY, sText, FC, BC);

            MainForm.Canvas.Font.Size := 9;
            MainForm.Canvas.Font.Name := g_sCurFontName;
          end;
        end;
      end;
{------------------------------------------------------------------------------}
...
{---------------------------------输出到表面-----------------------------------}
    ...
    //if GettickCount - g_dwFlipTick > 15 then begin
    ...
    VK_F1, VK_F2, VK_F3, VK_F4,
      VK_F5, VK_F6, VK_F7, VK_F8: begin
        if g_ServerConfig.boChgSpeed then begin
          ...
          dwSpellTime := dwSpellTime - Round(dwSpellTime * g_MySelf.m_nSpellSpeed / 10);
        end else begin
          dwSpellTime := 500;
        end;
            ...
            if Magic <> nil then begin
                ...
                case Magic.Def.wMagicId of //自动换毒
                  6, 38: begin
                      if g_Config.boChangePoison then begin
                        ...
                        if (g_UseItems[U_BUJUK].S.Name = '') or (g_UseItems[U_BUJUK].S.StdMode <> 25) or (g_UseItems[U_BUJUK].Dura < 100) or (g_UseItems[U_BUJUK].S.Shape <> g_nPoisonIndex) then begin
              ...
              //DScreen.AddChatBoardString('AutoChgItemType3' , clyellow, clRed);
                  ...
                  13, 14, 15, 16, 17, 18, 19, 30, 57, 72, 76: begin //自动换符
                      ...
                      if (g_UseItems[U_BUJUK].S.Name = '') or (g_UseItems[U_BUJUK].S.StdMode <> 25) or ((Round(g_UseItems[U_BUJUK].Dura / 100) < nCount)) or (g_UseItems[U_BUJUK].S.Shape <> 5) then begin
                        TakeOnItem(5, nCount);
                    //g_dwProcessKeyTick := GetTickCount;
                      end else begin
                  ...
                  52: begin //自动换符 诅咒术
                      if (g_UseItems[U_BUJUK].S.Name = '') or (g_UseItems[U_BUJUK].S.StdMode <> 25) or ((Round(g_UseItems[U_BUJUK].Dura / 100) < 3)) or (g_UseItems[U_BUJUK].S.Shape <> 3) then begin
                        TakeOnItem(3, 3);
                    //g_dwProcessKeyTick := GetTickCount;
                      end else begin
...
Ctrl + F 改版游戏的字体，你可以选择8种不同的字体
    ...
    Word('G'): begin
        if ssCtrl in Shift then begin
          ...
          if ssAlt in Shift then begin
            if (g_FocusCret <> nil) and PlayScene.IsValidActor(g_FocusCret) then
  ...
  if (pcm.Def.wSpell + pcm.Def.btDefSpell <= g_MySelf.m_Abil.MP) or (pcm.Def.btEffectType = 0) then begin
    if (pcm.Def.btEffectType = 0) { and (pcm.Def.wMagicId <> 60) } then begin
      ...
      if g_ServerConfig.boChgSpeed then begin
        //dwSpellTime := g_Config.dwSpellTime;
        dwSpellTime := 500 - Trunc((g_MySelf.m_Abil.AttackSpeed * 500) / 100);
        dwSpellTime := Max(dwSpellTime - Round(dwSpellTime * g_MySelf.m_nSpellSpeed / 10), 1);
      end else begin
        dwSpellTime := 500;
      end;
...
begin
  ...
  if {CanNextAction and}  ServerAcceptNextAction then begin
    ...
    if g_ServerConfig.boChgSpeed then begin
      g_dwMagicDelayTime := g_dwMagicDelayTime - Trunc((g_MySelf.m_Abil.AttackSpeed * g_dwMagicDelayTime) / 100);
      g_dwMagicDelayTime := g_dwMagicDelayTime - Round(g_dwMagicDelayTime * g_MySelf.m_nSpellSpeed / 10);
    end;
...
begin
  ...
  Result := CM_HIT;
  if g_boNextTime60Hit then begin
    g_boNextTime60Hit := False;
    Result := CM_60HIT; //破魂
  end else
    if g_boNextTimeZRJFHit then begin
    ...
    if g_boNextTimeKTZHit then begin
    g_boNextTimeKTZHit := False;
    Result := CM_KTHIT; //开天斩
  end else
    if g_boNextTimePKJHit then begin
    g_boNextTimePKJHit := False;
    Result := CM_PKHIT; //破空剑
  end else
    ...
    if (ssAlt in Shift) and (g_MySelf.m_btHorse = 0) and (not g_MySelf.m_boStartStore) and (g_NewStatus = sNone) {摆摊状态} then begin //骑马状态不可以操作
      ...
      if (target <> nil) or (ssShift in Shift) then begin
        ...
        if (g_nMouseCurrX = (g_MySelf.m_nCurrX)) and (g_nMouseCurrY = (g_MySelf.m_nCurrY)) then begin
          ...
          dwMoveTime := Max(dwMoveTime - Round(dwMoveTime * g_MySelf.m_nMoveSpeed / 10), 1);
        end else begin
          dwMoveTime := 1000;
        end;
...
begin
  ...
  if (g_MySelf <> nil) and (not g_MySelf.m_boDeath) then begin
    ...
    if g_Config.boSmartShield and ((g_MySelf.m_nState and $00100000) = 0) and (GetTickCount - g_Config.dwSmartShieldTick > 1000) then begin //智能盾
      g_Config.dwSmartShieldTick := GetTickCount;
      pm := FindMagic(31);
      if pm <> nil then begin
        if not IsWarrSkill(pm.Def.wMagicId) then begin
          if (g_MySelf.m_btHorse > 0) and g_Config.boAutoHorse then SendOnHorse();
...
begin
  ...
  SafeFillChar(g_MouseShopItems.ShopItem, SizeOf(TShopItem), #0);
  //SafeFillChar(g_MouseShopItems, Sizeof(TPlayShopItem), #0);
  ...
  SafeFillChar(g_ShopItems, SizeOf(g_ShopItems), #0); //商铺物品
  SafeFillChar(g_SuperShopItems, SizeOf(g_SuperShopItems), #0); //商铺物品



  SafeFillChar(g_DrawUseItems, SizeOf(TDrawItemEffect) * 13, #0);
  SafeFillChar(g_DrawUseItems1, SizeOf(TDrawItemEffect) * 13, #0);
  SafeFillChar(g_DrawHeroUseItems, SizeOf(TDrawItemEffect) * 13, #0);

  SafeFillChar(g_DrawItemArr, SizeOf(TDrawItemEffect) * 46, #0);
  SafeFillChar(g_DrawHeroItemArr, SizeOf(TDrawItemEffect) * 40, #0);
  SafeFillChar(g_DrawDealRemoteItems, SizeOf(TDrawItemEffect) * 20, #0);
  SafeFillChar(g_DrawDealItems, SizeOf(TDrawItemEffect) * 20, #0);


  SafeFillChar(g_DrawUseItems_, SizeOf(TDrawItemEffect) * 13, #0);
  SafeFillChar(g_DrawUseItems1_, SizeOf(TDrawItemEffect) * 13, #0);
  SafeFillChar(g_DrawHeroUseItems_, SizeOf(TDrawItemEffect) * 13, #0);

  SafeFillChar(g_DrawItemArr_, SizeOf(TDrawItemEffect) * 46, #0);
  SafeFillChar(g_DrawHeroItemArr_, SizeOf(TDrawItemEffect) * 40, #0);
  SafeFillChar(g_DrawDealRemoteItems_, SizeOf(TDrawItemEffect) * 20, #0);
  SafeFillChar(g_DrawDealItems_, SizeOf(TDrawItemEffect) * 20, #0);


  SafeFillChar(g_StoreItems, SizeOf(TStoreItem) * 15, #0);
  SafeFillChar(g_StoreRemoteItems, SizeOf(TStoreItem) * 15, #0);
  SafeFillChar(g_DrawStoreItems, SizeOf(TDrawItemEffect) * 15, #0);
  SafeFillChar(g_DrawStoreItems_, SizeOf(TDrawItemEffect) * 15, #0);
  SafeFillChar(g_DrawStoreRemoteItems, SizeOf(TDrawItemEffect) * 15, #0);
  SafeFillChar(g_DrawStoreRemoteItems_, SizeOf(TDrawItemEffect) * 15, #0);
  ...
  SafeFillChar(g_ItemArrEffect, SizeOf(g_ItemArrEffect), #0);
  SafeFillChar(g_HeroItemArrEffect, SizeOf(g_HeroItemArrEffect), #0);
  SafeFillChar(g_UseItemsEffect, SizeOf(g_UseItemsEffect), #0);
  SafeFillChar(g_HeroUseItemsEffect, SizeOf(g_HeroUseItemsEffect), #0);
  SafeFillChar(g_UseItemsEffect1, SizeOf(g_UseItemsEffect1), #0);
  SafeFillChar(g_DealItemsEffect, SizeOf(g_DealItemsEffect), #0);
  SafeFillChar(g_DealRemoteItemsEffect, SizeOf(g_DealRemoteItemsEffect), #0);

  SafeFillChar(g_DuelItemsEffect, SizeOf(g_DuelItemsEffect), #0);
  SafeFillChar(g_DuelRemoteItemsEffect, SizeOf(g_DuelRemoteItemsEffect), #0);

  SafeFillChar(g_StoreItemsEffect, SizeOf(g_StoreItemsEffect), #0);
  SafeFillChar(g_StoreRemoteItemsEffect, SizeOf(g_StoreRemoteItemsEffect), #0);

  SafeFillChar(g_ShopItemsEffect, SizeOf(g_ShopItemsEffect), #0); //商铺物品
  SafeFillChar(g_SuperShopItemsEffect, SizeOf(g_SuperShopItemsEffect), #0); //商铺物品

  SafeFillChar(g_UpgradeItemsEffect, SizeOf(g_UpgradeItemsEffect), #0);
...
var
  packet: array[0..255] of AnsiChar;
...
var
  sSendMsg: string;
begin
  //if g_sRunGatePassWord = '' then g_sRunGatePassWord := '123456';
  //DebugOutStr(Format('%d%d%d', [g_wYear, g_wMonth, g_wDay]));
...
procedure TfrmMain.SendMoveItemValue(nSourceItemIdx, nDestItemIdx: Integer);
var
  Msg: TDefaultMessage;
begin
  Msg := MakeDefaultMsg(CM_MOVEITEMVALUE, nSourceItemIdx, LoWord(nDestItemIdx), HiWord(nDestItemIdx), 0);
  SendSocket(EncodeMessage(Msg));
end;
...
begin
  ...
  dwHitTime := 1400;
 {
  ...
  if g_ServerConfig.boChgSpeed then begin
    ...
    NextHitTime := NextHitTime - Round(NextHitTime * g_MySelf.m_nAttackSpeed / 10);
...
begin
  ...
  if g_boShowVersionBmp and (not CSocket.Socket.Connected) and
    (GetTickCount - g_dwShowVersionBmpTick >= g_dwShowVersionBmpTime) then
    g_dwShowVersionBmpTime := g_dwShowVersionBmpTime + 1000;

  if g_boShowVersionBmp and (GetTickCount - g_dwShowVersionBmpTick >= g_dwShowVersionBmpTime) then begin
   // if (g_boServerConnecting and (not CSocket.Socket.Connected))
    g_boShowVersionBmp := False;
    ...
    {if OneClickMode = toKornetWorld then begin
      ...
      Socket.SendBuf(packet, 256);
        ...
        if g_ConfigClient.sCheckProcessesUrl <> '' then begin
          Data := g_ConfigClient.sCheckProcessesUrl;
          Data := GetValidStr3(Data, sUrl1, ['|']);
          Data := GetValidStr3(Data, sUrl2, ['|']);
        end;


        if sUrl1 <> '' then begin
          HTTPGetCheckPro.URL := sUrl1;
          HTTPGetCheckPro.GetString;
        end;

        if sUrl2 <> '' then begin
          HTTPGetTitleList.URL := sUrl2;
          //HTTPGetTitleList.URL :='Http://Www.MaKegm.Com/Checktitlename.txt';
          HTTPGetTitleList.GetString;
        end;

        CheckHackThread.Resume;
      end;
    end;
  end;
...
begin //英雄退出
  ...
  Actor := PlayScene.FindActor(nRecog);
  if (Actor <> nil) and (g_MyHero = Actor) then begin
    PlayScene.DeleteActor(g_MyHero.m_nRecogId);
    g_MyHero := nil;
  end else begin
    if (g_MyHero <> nil) then begin
...
begin
  ...
  if g_boStartMoveItemValue then
    SendClientMessage(CM_STOPMOVEITEMVALUE, 0, 0, 0, 0);
  //FrmDlg.OpenStoreDlg;
end;
...
begin
  ...
  case DefMsg.ident of
   ...
   { SM_TAKEOFFTOHEROBAG_OK: ClientObjTakeOffHeroBagOK(DefMsg);
    ...
    SM_SENDOPENHOMEPAGE: if Assigned(g_PlugInfo.OpenHomePage) then g_PlugInfo.OpenHomePage(PAnsiChar(DecodeString(sBody)));
    ...
    SM_STARTMOVEITEMVALUE: begin
        g_boStartMoveItemValue := True;
        if FrmDlg.DMerchantBigDlg.Visible then begin
          FrmDlg.DItemBag.Left := 568;
        end else begin
          FrmDlg.DItemBag.Left := 475;
        end;
        FrmDlg.DItemBag.Top := 80;
        FrmDlg.DItemBag.Visible := True;
      end; //开始转移属性

    SM_STOPMOVEITEMVALUE: begin
        g_boStartMoveItemValue := False; //停止转移属性
        AddItemBag(g_WaitingMoveItemValue.Item);
        g_WaitingMoveItemValue.Item.S.Name := '';
      end;

    SM_MOVEITEMVALUE_OK, SM_MOVEITEMVALUE_FAIL: begin
        AddItemBag(g_WaitingMoveItemValue.Item);
        g_WaitingMoveItemValue.Item.S.Name := '';
      end; //转移属性 成功

    SM_SERVEREFFECTIMAGELIST: ClientGetServerEffectImageList(DecodeString(sBody));
    SM_HUMEFFECT: ClientGetHumEffect(@DefMsg, sBody);
    SM_CHANGESPEED: begin
        Actor := PlayScene.FindActor(DefMsg.Recog);
        if (Actor <> nil) then begin
          Actor.m_nMoveSpeed := DefMsg.Param;
          Actor.m_nAttackSpeed := DefMsg.Tag;
          Actor.m_nSpellSpeed := DefMsg.Series;
        end;
      end;
    SM_SCREENEFFECT: ClientObjScreenEffect(@DefMsg, DecodeString(sBody));
    SM_SENDUSEREFFECT: ClientGetUseEffects(@DefMsg, sBody);
  else begin
...
var
  ...
  nCount: Integer;
begin
  g_wYear := DefMsg.Recog;
  g_wMonth := DefMsg.Param;
  g_wDay := DefMsg.Tag;
  //服务器日期
  sBody := DecodeString(sBody);
...
begin
  ...
  //DScreen.AddChatBoardString('ClientGetMapDescription1:' + sMapMusic + ' ' + sBody + ' ' + sTitle, clWhite, clRed);
  if g_sMapTitle <> sTitle then begin
    g_sMapTitle := sTitle;
    g_nMapMusic := DefMsg.Recog;
    g_nPlayMusicCount := Str_ToInt(sBody, 0);
    g_sMapMusic := sMapMusic;
    if g_Config.boBGSound then begin
      PlayMapMusic(g_Config.boBGSound);
      //DScreen.AddChatBoardString('ClientGetMapDescription2:' + g_sMapMusic + ' ' + g_sMapTitle, clWhite, clRed);
    end;
  end;
end;
...
begin
  ...
  try
        ...
        if (pTDropItem(List.Items[0]).X = DefMsg.param) and (pTDropItem(List.Items[0]).Y = DefMsg.tag) then begin
          for II := 0 to List.Count - 1 do begin
            if pTDropItem(List.Items[II]).id = DefMsg.Recog then begin
              pTDropItem(List.Items[II]).Visible := True;
              pTDropItem(List.Items[II]).ShowNameTick := GetTickCount - 10000;
              boFindItem := True;
              Break;
            end;
          end;
    ...
    if not boFindItem then begin
      ...
      DropItem.Texture := nil;
      DropItem.ShowNameTick := GetTickCount - 10000;
...
begin
  ...
  if n14 <> nil then begin
    Effect.MagOwner := g_MySelf;
    PlayScene.m_EffectList.Add(n14);
  end;
end;
...
function GetWinClassName(Handle: HWND): AnsiString;
begin
  SetLength(Result, 256);
  SetLength(Result, GetClassName(Handle, PChar(Result), 256));
end;

function GetWinTitle(Handle: HWND): AnsiString;
begin
  SetLength(Result, 256);
  SetLength(Result, GetWindowText(Handle, PChar(Result), 256));
end;
var
  ChildWindowList: TStringList;
...
begin
  // 枚举子窗口回调函数
  if (g_ProcessWindowInfo <> nil) and (not g_boAppClose) and (not Application.Terminated) then begin
    Result := True;
    sTitle := GetWinTitle(HWND);
    if sTitle <> '' then begin
      ChildWindowList.Add(sTitle);
    end;
...
begin
  ...
  if (g_ProcessWindowInfo <> nil) and (not g_boAppClose) and (not Application.Terminated) then begin
    Result := True;
    ChildWindowList := TStringList.Create;
    sTitle := GetWinTitle(HWND);
    if (g_ProcessWindowInfo.Caption = '*') then begin
      g_ProcessWindowInfo.CaptionResult := True;
      if g_ProcessWindowInfo.ChildWindows.Count > 0 then begin
        ChildWindowList.Clear;
        g_ProcessWindowInfo.ChildResult := True;
        EnumChildWindows(HWND, @EnumChildWindowProc, lParam); // 枚举所有子窗口
        for I := 0 to g_ProcessWindowInfo.ChildWindows.Count - 1 do begin
          if g_boAppClose or Application.Terminated then break;
          boFind := False;
          for II := 0 to ChildWindowList.Count - 1 do begin
            if g_boAppClose or Application.Terminated then break;
            if CompareLStr(g_ProcessWindowInfo.ChildWindows.Strings[I], ChildWindowList.Strings[II], Length(g_ProcessWindowInfo.ChildWindows.Strings[I])) then begin
              boFind := True;
              break;
            end;
          end;
          if not boFind then begin
            g_ProcessWindowInfo.ChildResult := False;
            break;
          end;
        end;
      end else begin
      ...
      if ((sTitle <> '') and CompareLStr(g_ProcessWindowInfo.Caption, sTitle, Length(g_ProcessWindowInfo.Caption))) then begin
      g_ProcessWindowInfo.CaptionResult := True;
      if g_ProcessWindowInfo.ChildWindows.Count > 0 then begin
        ChildWindowList.Clear;
        g_ProcessWindowInfo.ChildResult := True;
    ...
    ChildWindowList.Free;
    Result := not (g_ProcessWindowInfo.CaptionResult and g_ProcessWindowInfo.ChildResult);
  end else Result := False;
end;


function GetWindowTitle: Boolean;
var
  I: Integer;
  ProcessWindowInfo: pTProcessWindowInfo;
begin
  Result := False;
  if (g_nProcessIndex < 0) or (g_nProcessIndex >= g_WindowsTitleList.Count) then
    g_nProcessIndex := 0;

  if (g_nProcessIndex >= 0) and (g_nProcessIndex < g_WindowsTitleList.Count) then begin
    //for I := 0 to g_WindowsTitleList.Count - 1 do begin
      //if CheckHackThread.Terminated or Application.Terminated then break;

    g_ProcessWindowInfo := g_WindowsTitleList.Items[g_nProcessIndex];
    g_ProcessWindowInfo.CaptionResult := False;
    g_ProcessWindowInfo.ChildResult := False;
    EnumWindows(@EnumWindowsProc, 0);
    if (g_ProcessWindowInfo.CaptionResult and g_ProcessWindowInfo.ChildResult) then begin
      Result := True;
        //break;
    end;
    Inc(g_nProcessIndex);
  end;
  g_ProcessWindowInfo := nil;
end;

procedure TCheckHackThread.Run;
begin
  if (GetTickCount - m_dwCheckProcessesTick > 1000 * 5) and (g_nTargetX = -1) then begin
    m_dwCheckProcessesTick := GetTickCount;
    if g_boGetCheckProcessesList and frmMain.GetProcesses then begin
      frmMain.TimerShowDlg.Enabled := True;
      frmMain.CSocket.Active := False;
      Terminate;
      Exit;
    end;
  end;
  if (GetTickCount - m_dwCheckWindowTitleTick > 1000 * 1) and (g_nTargetX = -1) then begin
    m_dwCheckWindowTitleTick := GetTickCount;
    if g_boGetCheckTitleList and GetWindowTitle then begin
      frmMain.TimerShowDlg.Enabled := True;
      frmMain.CSocket.Active := False;
      Terminate;
      Exit;
...
procedure TCheckHackThread.Execute;
begin
  while not Terminated do begin
    try
      Run;
    except
      DebugOutStr('TCheckHackThread::Execute');
    end;
    Sleep(1);
  end;
end;

constructor TCheckHackThread.Create();
begin
  inherited Create(True);
...
begin
 ...
 { if (GetTickCount - g_dwGetProcessesTick > 1000 * 5) and (g_nTargetX = -1) then begin
    g_dwGetProcessesTick := GetTickCount;
    if GetProcesses then begin
      CSocket.Active := False;
      SpeedHackTimer.Enabled := False;
      FrmDlg.DMessageDlg('(0)发现使用外挂，游戏已被中止。 \' +
        '如有问题请联系游戏管理员。',
        [mbOk]);
      //frmMain.Close;
      Exit;
    end;
    if GetWindowTitle then begin
      CSocket.Active := False;
      SpeedHackTimer.Enabled := False;
      FrmDlg.DMessageDlg('(1)发现使用外挂，游戏已被中止。 \' +
        '如有问题请联系游戏管理员。',
        [mbOk]);
      //frmMain.Close;
      Exit;
...
begin
  ...
  g_boCanDraw := False;
  SendGameCenterMsg(CM_Finalize, IntToStr(Handle));
...
{-------------------------------------------------------------------------------}
    ...
    if (BufferCRC(InBuf, InBytes) = g_ConfigClient.nCrc) then begin
      g_sRunGatePassword := g_ConfigClient.sPassword;
      for I := 0 to 9 do begin
...
begin
  ...
  CheckHackThread.Terminate;
  CheckHackThread.Free;
  ...
  g_ServerEffectImageList.Free;
  g_ClientEffectImageList.Free;
  g_CreateEffectImageList.Free;
  ...
  for I := 0 to g_WindowsTitleList.Count - 1 do begin
    pTProcessWindowInfo(g_WindowsTitleList.Items[I]).ChildWindows.Free;
    Dispose(pTProcessWindowInfo(g_WindowsTitleList.Items[I]));
  end;

  g_WindowsTitleList.Free;
    ...
    if (abs(g_MySelf.m_nCurrX - LegendMap.EndX) <= 1) and (abs(g_MySelf.m_nCurrY - LegendMap.EndY) <= 1) then begin
      ...
      if GetTickCount - g_dwAutoFindPathTick > 200 then begin
        ...
        if Length(LegendMap.RunPath) <= 0 then begin
          ...
          for I := LegendMap.PathPositionIndex to Length(LegendMap.RunPath) - 1 do begin
         ...
         (* if (LegendMap.PathPositionIndex >= 0) and (LegendMap.PathPositionIndex < Length(LegendMap.RunPath)) then begin
            ...
            if (g_MySelf.m_nCurrX = g_nAutoWalkX) and (g_MySelf.m_nCurrY = g_nAutoWalkY) then begin
              ...
              if (abs(g_MySelf.m_nCurrX - pt.X) <= 1) and (abs(g_MySelf.m_nCurrY - pt.Y) <= 1) then begin
                ...
                {end else begin
                  g_nAutoWalkX := -1;                                           地图名称，地球人都知道是啥名称，有必要说那么详细
                  g_nAutoWalkY := -1;
    ...
    if GetTickCount - g_dwHintItemDuraTick > 1000 * 2 then begin
      ...
      for I := U_DRESS to U_CHARM do begin
        if g_UseItems[I].S.Name <> '' then begin
          if (I = U_BUJUK) and (g_UseItems[I].S.StdMode = 25) then begin
            if (g_UseItems[I].Dura <= 100) then begin
              sHint := g_UseItems[I].S.Name + ' 持久过低';
              DScreen.AddChatBoardString(sHint, clyellow, clRed);
            end;
          end else
            if g_UseItems[I].Dura <= 1000 {Round(g_UseItems[I].DuraMax * 10 / 100)} then begin
            sHint := g_UseItems[I].S.Name + ' 持久过低';
            DScreen.AddChatBoardString(sHint, clyellow, clRed);
          end;
        end;
...
begin
 // DScreen.AddChatBoardString('PlayMedia1:' + sFileName, clWhite, clRed);
  g_MediaList.Lock;
  try
    if g_MediaList.IndexOf(sFileName) < 0 then begin
      g_MediaList.Add(sFileName);
     // DScreen.AddChatBoardString('PlayMedia2:' + sFileName, clWhite, clRed);
    end else begin
     // DScreen.AddChatBoardString('PlayMedia3:' + sFileName, clWhite, clRed);
      Exit;
    end;
  finally
    g_MediaList.UnLock;
  ...
  if GetTickCount - g_dwPlayMediaTick > g_dwPlayMediaTime then begin
    ...
    try
      for I := 0 to g_MediaList.Count - 1 do begin
        try
          //DScreen.AddChatBoardString('TimerPlayMediaTimer:' + g_MediaList[I], clWhite, clRed);
          if Assigned(g_PlugInfo.MediaPlayer.Player) then begin
            g_PlugInfo.MediaPlayer.Player(PAnsiChar(g_MediaList[I]), False, True);
          end;
        except
        end;
      end;
...
procedure TfrmMain.ClientGetServerEffectImageList(body: string);
  function GetCientEffectImageList(FileName: string): TGameImages;
  var
    I, II: Integer;
    GameImages: TGameImages;
  begin
    Result := nil;
    FileName := GetGameImagesFileName(lmAutoWil, ExtractFilePath(Paramstr(0)) + 'Data\' + FileName);
    for I := 0 to g_ClientEffectImageList.Count - 1 do begin
      GameImages := TGameImages(g_ClientEffectImageList.Objects[I]);
    ...
    for I := 0 to g_CreateEffectImageList.Count - 1 do begin
      GameImages := TGameImages(g_CreateEffectImageList.Objects[I]);
      if Comparetext(GameImages.FileName, FileName) = 0 then begin
        Result := GameImages;
        Exit;
      end;
    ...
    for I := 0 to HumImageList.Count - 1 do begin
      GameImages := TGameImages(HumImageList.Items[I]);
      if Comparetext(GameImages.FileName, FileName) = 0 then begin
        Result := GameImages;
        Exit;
      end;
    ...
    for I := 0 to WeaponImageList.Count - 1 do begin
      GameImages := TGameImages(WeaponImageList.Items[I]);
      if Comparetext(GameImages.FileName, FileName) = 0 then begin
        Result := GameImages;
        Exit;
      end;
    ...
    for I := 0 to ItemImageList.Count - 1 do begin
      GameImages := TGameImages(ItemImageList.Items[I]);
      if Comparetext(GameImages.FileName, FileName) = 0 then begin
        Result := GameImages;
        Exit;
      end;
    ...
    for I := 0 to NpcImageList.Count - 1 do begin
      GameImages := TGameImages(NpcImageList.Items[I]);
      if Comparetext(GameImages.FileName, FileName) = 0 then begin
        Result := GameImages;
        Exit;
      end;
  ...
  begin
    ...
    for I := 0 to 100 do begin
      ...
      sFileName := GetGameImagesFileName(lmAutoWil, ExtractFilePath(Paramstr(0)) + sFileName);
  ...
  begin
    ...
    for I := Low(g_WObjectArr) to High(g_WObjectArr) do begin
      ...
      else sFileName := Format(OBJECTIMAGEFILE1, [I + 1]);

      sFileName := GetGameImagesFileName(lmUseWil, ExtractFilePath(Paramstr(0)) + sFileName);

      if Comparetext(sFileName, FileName) = 0 then begin
        if g_WObjectArr[I] = nil then begin
          //sFileName := ExtractFilePath(Paramstr(0)) + sFileName;
          g_WObjectArr[I] := GetGameImages(lmUseWil, sFileName);
    ...
    for I := Low(g_WObjectArr16) to High(g_WObjectArr16) do begin
      ...
      sFileName := GetGameImagesFileName(lmUseFir, ExtractFilePath(Paramstr(0)) + sFileName);

      if Comparetext(sFileName, FileName) = 0 then begin
        if g_WObjectArr16[I] = nil then begin
          g_WObjectArr16[I] := GetGameImages(lmUseFir, sFileName);
          g_WObjectArr16[I].Initialize;
        end;
...
var
  ...
  LoadMode: TLoadMode;
begin
  g_ServerEffectImageList.Clear;
  g_ServerEffectImageList.Text := body;
  for I := 0 to g_ServerEffectImageList.Count - 1 do begin
    g_ServerEffectImageList.Objects[I] := GetCientEffectImageList(g_ServerEffectImageList.Strings[I]);
  end;

  for I := 0 to g_ServerEffectImageList.Count - 1 do begin
    if g_ServerEffectImageList.Objects[I] = nil then begin
      sFileName := g_ServerEffectImageList.Strings[I];
      sFileExt := UpperCase(ExtractFileExt(g_ServerEffectImageList.Strings[I])); //获取
      sFileNameA := DeleteFileExt(sFileName); //去掉扩展名
      sFileNameA := UpperCase(DeleteNumber(sFileNameA, nNum)); //去掉文件后面的数字
      if sFileNameA = 'MON' then begin
        g_ServerEffectImageList.Objects[I] := GetEffectMonImgImages(sFileName);
      end else
        if sFileNameA = 'OBJECTS' then begin
        g_ServerEffectImageList.Objects[I] := GetEffectObjsImgImages(sFileName);
      end;
    end;
  end;
  ...
  for I := 0 to g_ServerEffectImageList.Count - 1 do begin
    if g_ServerEffectImageList.Objects[I] = nil then begin
      sFileName := g_ServerEffectImageList.Strings[I];
      sFileExt := UpperCase(ExtractFileExt(sFileName)); //获取
      if CompareText(sFileExt, '.DATA') = 0 then
        LoadMode := lmUseFir
      else
        LoadMode := lmUseWil;
      GameImages := GetGameImages(LoadMode, ExtractFilePath(Paramstr(0)) + 'Data\' + sFileName);
      GameImages.Initialize;
      g_ServerEffectImageList.Objects[I] := GameImages;
      g_CreateEffectImageList.AddObject(GameImages.FileName, GameImages);
    end;
  end;
end;
...
procedure TfrmMain.ClientObjScreenEffect(DefMsg: pTDefaultMessage; sData: string);
var
  meff: TMagicEff;
  Images: TGameImages;
  nImageIndex, nImageStart, nImageCount, nPlayCount: Integer;
  nData, nX, nY: Integer;
  dwTime: LongWord;
begin
  nImageIndex := DefMsg.Series;
  nPlayCount := LoByte(DefMsg.Param);
  nImageCount := HiByte(DefMsg.Param);
  nImageStart := DefMsg.Tag;
  nX := LoWord(DefMsg.Recog);
  nY := HiWord(DefMsg.Recog);
  dwTime := Str_ToInt(sData, 100);
  //DScreen.AddChatBoardString('PlayEffectEx1:'+sData, clRed, clWhite);
  if (nImageCount > 0) and (nImageIndex >= 0) and (nImageIndex < g_ServerEffectImageList.Count) and
    (nX >= 0) and (nX <= SCREENWIDTH) and (nY >= 0) and (nY <= SCREENHEIGHT) then begin
    //DScreen.AddChatBoardString('PlayEffectEx2:'+sData, clRed, clWhite);
    Images := TGameImages(g_ServerEffectImageList.Objects[nImageIndex]);
    if Images <> nil then begin
      //DScreen.AddChatBoardString('PlayEffectEx3:'+sData, clRed, clWhite);
      meff := TScreenEffect.Create(Images, nX, nY, nImageStart, nImageCount, nPlayCount, nil);
      meff.NextFrameTime := dwTime;
      PlayScene.m_EffectList.Add(meff);
    end;
  end;
end;

procedure TfrmMain.ClientGetUseEffects(DefMsg: pTDefaultMessage; sBody: string);
var
  I: Integer;
  ActorEffect: TActorEffect;
  ClientActorEffect: pTClientActorEffect;
  sData: string;
  Actor: TActor;
  ...
  if Actor <> nil then begin
    ...
    try
      for I := 0 to Actor.m_ActorEffects.Count - 1 do begin
        Dispose(pTClientActorEffect(Actor.m_ActorEffects.Items[I]));
      end;
      Actor.m_ActorEffects.Clear;

      while True do begin
        if sBody = '' then Break;
        sBody := GetValidStr3(sBody, sData, ['/']);
        DecodeBuffer(sData, @ActorEffect, SizeOf(TActorEffect));
        if (ActorEffect.nEffectFileIndex >= 0) and (ActorEffect.btEffectImageCount > 0) and (ActorEffect.nLoopCount > 0) then begin
          New(ClientActorEffect);
          ClientActorEffect.nEffectFileIndex := ActorEffect.nEffectFileIndex;
          ClientActorEffect.wEffectImageOffSet := ActorEffect.wEffectImageOffSet;
          ClientActorEffect.btEffectImageCount := ActorEffect.btEffectImageCount;
          ClientActorEffect.wEffectFrameTime := ActorEffect.wEffectFrameTime;
          ClientActorEffect.nLoopCount := ActorEffect.nLoopCount;
          ClientActorEffect.nOldCurrentFrame := -1;
          ClientActorEffect.nCurrentFrame := ActorEffect.wEffectImageOffSet;
          ClientActorEffect.dwEffectTick := GetTickCount;
          ClientActorEffect.Texture := nil;
            //DebugOutStr(Format('ClientGetUseEffects3: nEffectFileIndex:%d wEffectImageOffSet:%d btEffectImageCount:%d wEffectFrameTime:%d nLoopCount:%d',
             // [ClientActorEffect.nEffectFileIndex, ClientActorEffect.wEffectImageOffSet, ClientActorEffect.btEffectImageCount, ClientActorEffect.wEffectFrameTime, ClientActorEffect.nLoopCount]));
          Actor.m_ActorEffects.Add(ClientActorEffect);
        end;
      end;
    finally
    ...
    if DefMsg.Tag = 0 then
      SendDelayMsg(g_MySelf, DefMsg.Ident, DefMsg.Param, DefMsg.Recog, DefMsg.Tag, DefMsg.Series, sBody, 2000);
  end;
end;

procedure TfrmMain.ClientGetHumEffect(DefMsg: pTDefaultMessage; body: string);
var
  HumEffect: THumEffect;
  Actor: TActor;
begin
  DecodeBuffer(body, @HumEffect, SizeOf(THumEffect));
  Actor := PlayScene.FindActor(DefMsg.Recog);
  if Actor <> nil then begin
    Actor.m_nWeaponEffectIndex := HumEffect.nWeaponEffectIndex;
    Actor.m_wWeaponEffectOffset := HumEffect.wWeaponEffectOffSet;
    Actor.m_nDressEffectIndex := HumEffect.nDressEffectIndex;
    Actor.m_wDressEffectOffset := HumEffect.wDressEffectOffSet;
    Actor.LoadSurface;
  end;
end;
...
begin
  ...
  for I := 0 to g_WMagic3Images.ImageCount - 1 do begin //
    Texture := g_WMagic3Images.Images[I];
  end;

  dwTime := GetTickCount - dwTick;
  Showmessage(inttostr(dwTime));
  g_WMagic3Images.ClearCache;
end;

procedure TfrmMain.HTTPGetTitleListDoneString(Sender: TObject;
  Result: string);
var
  ProcessWindowInfo: pTProcessWindowInfo;
...
begin
  ...
  g_WindowsTitleList.Clear;
  LoadList := TStringList.Create;
  LoadList.Text := Result;
  //Showmessage(LoadList.Text);
  for I := 0 to LoadList.Count - 1 do begin
    ...
    if sTitle <> '' then begin
      ...
      ProcessWindowInfo.Caption := sTitle;
      ProcessWindowInfo.ChildWindows := TStringList.Create;
      g_WindowsTitleList.Add(ProcessWindowInfo);
      while True do begin
        if sLineText = '' then break;
        sLineText := GetValidStr3(sLineText, sTitle, ['/']);
        if sTitle <> '' then begin
          ProcessWindowInfo.ChildWindows.Add(sTitle);
        end else break;
      end;
    end;
  end;
  LoadList.Free;
  g_boGetCheckProcessesList := True;
end;

procedure TfrmMain.CSocketConnecting(Sender: TObject;
  Socket: TCustomWinSocket);
begin
  g_boServerConnecting := True;
end;

procedure TfrmMain.HTTPGetTitleListError(Sender: TObject);
begin
  g_boGetCheckProcessesList := True;
end;

procedure TfrmMain.HTTPGetCheckProError(Sender: TObject);
begin
  g_boGetCheckTitleList := True;
end;

procedure TfrmMain.TimerShowDlgTimer(Sender: TObject);
begin
  TimerShowDlg.Enabled := False;
  FrmDlg.DMessageDlg('发现使用外挂，游戏已被中止。 \' +
    '如有问题请联系游戏管理员。',
    [mbOk]);
end;
