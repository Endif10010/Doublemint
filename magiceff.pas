...
type
  ...
  TScreenEffect = class(TMagicEff)
    m_nMaxCount: Integer;
    m_nPlayCount: Integer;
  public
    constructor Create(Images: TGameImages; nCurrX, nCurrY, nImageStart, nImageCount, nPlayCount: Integer; Target: TObject);
    function Run: Boolean; override;
...
constructor TScreenEffect.Create(Images: TGameImages; nCurrX, nCurrY, nImageStart, nImageCount, nPlayCount: Integer; Target: TObject);
...
procedure TScreenEffect.DrawEff(Surface: TTexture);
var
  img: Integer;
  d: TTexture;
  shx, shy: Integer;
...
function TScreenEffect.Run: Boolean;
begin
  Result := Shift;
...
function TScreenEffect.Shift: Boolean;
  function OverThrough(olddir, newdir: Integer): Boolean;
  begin
    Result := False;
    if abs(olddir - newdir) >= 2 then begin
...
begin
  ...
  if GetTickCount - steptime > LongWord(NextFrameTime) then begin
    ...
    if not OutofOil then begin
      if (MagOwner = nil) or (abs(rx - TActor(MagOwner).m_nRx) >= 5) or (abs(ry - TActor(MagOwner).m_nRy) >= 5) or (GetTickCount - firetime > 800) then
        OutofOil := True;
